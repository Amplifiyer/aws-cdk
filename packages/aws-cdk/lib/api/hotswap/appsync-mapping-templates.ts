...
export async function isHotswappableAppSyncChange(
  ...
  const isFunction =
    change.newValue.Type === "AWS::AppSync::FunctionConfiguration";
  const isGraphQLSchema =
    change.newValue.Type === "AWS::AppSync::GraphQLSchema";
  ...
  const namesOfHotswappableChanges = Object.keys(
    ...
    ret.push({
      ...
      apply: async (sdk: ISDK) => {
        ...
        const sdkProperties: { [name: string]: any } = {
          ...
          requestMappingTemplate:
            change.newValue.Properties?.RequestMappingTemplate,
          responseMappingTemplate:
            change.newValue.Properties?.ResponseMappingTemplate,
        };
        const evaluatedResourceProperties =
          await evaluateCfnTemplate.evaluateCfnExpression(sdkProperties);
        const sdkRequestObject = transformObjectKeys(
          evaluatedResourceProperties,
          lowerCaseFirstCharacter
        );
        ...
        if (isResolver) {
          ...
          const { functions } = await sdk
            .appsync()
            .listFunctions({ apiId: sdkRequestObject.apiId })
            .promise();
          const { functionId } =
            functions?.find((fn) => fn.name === physicalName) ?? {};
          await sdk
            .appsync()
            .updateFunction({
              ...sdkRequestObject,
          ...
          let schemaCreationResponse: GetSchemaCreationStatusResponse =
            await sdk.appsync().startSchemaCreation(sdkRequestObject).promise();
          while (
            schemaCreationResponse.status &&
            ["PROCESSING", "DELETING"].some(
              (status) => status === schemaCreationResponse.status
            )
          ) {
            ...
            schemaCreationResponse = await sdk
              .appsync()
